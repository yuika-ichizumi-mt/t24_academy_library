...
public class RentalManageService {
    ...
    public long countByStockIdAndStatusAndTermsIn(String stockId, Date expectedReturnOn, Date expectedRentalOn) {
        return this.rentalManageRepository.countByStockIdAndStatusAndTermsIn(stockId, expectedReturnOn,
                expectedRentalOn);
    }

    @Transactional
    public long countByStockIdAndStatusAndIdNotAndTermsIn(String stockId, Long id, Date expectedReturnOn,
            Date expectedRentalOn) {
        return this.rentalManageRepository.countByStockIdAndStatusAndIdNotAndTermsIn(stockId, id, expectedReturnOn,
                expectedRentalOn);
    }
    ...
    public String isStatusError(Integer beforeStatus, Integer afterStatus, LocalDate newexpectedRentalOn,
            LocalDate currentDate) { // 引数＝使う変数（オブジェクト）
        ...
        if (beforeStatus == RentalStatus.RETURNED.getValue() || beforeStatus == RentalStatus.CANCELED.getValue()) {
            ...
            // 貸出待ちから返却済みに変更できない
        } else if (beforeStatus == RentalStatus.RENT_WAIT.getValue()
                && afterStatus == RentalStatus.RETURNED.getValue()) {
            return "このステータスは無効です";
            // 貸出中から貸出待ちに変更できない
        } else if (beforeStatus == RentalStatus.RENTAlING.getValue()
                && afterStatus == RentalStatus.RENT_WAIT.getValue()) {
            return "このステータスは無効です";
            // 貸出中からキャンセルに変更できない
        } else if (beforeStatus == RentalStatus.RENTAlING.getValue()
                && afterStatus == RentalStatus.CANCELED.getValue()) {
            return "このステータスは無効です";
            // 貸出待ちから貸出中に変更するのは、貸出予定日当日である必要がある
        } else if (!newexpectedRentalOn.isEqual(currentDate) && beforeStatus == RentalStatus.RENT_WAIT.getValue()
                && afterStatus == RentalStatus.RENTAlING.getValue()) {
